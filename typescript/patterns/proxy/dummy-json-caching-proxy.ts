import {
  DummyThirdPartyApiService,
  TProduct,
  TRecipe,
} from "./dummy-3rd-party-api-service";
...
export class DummyJsonCachingProxy implements DummyJsonApi {
  private cachingLayer: Map<string, Array<TProduct>> = new Map();
  ...
  async getAllProducts() {
    if (this.cachingLayer.has(PRODUCTS_KEY)) {
      ...
      return this.cachingLayer.get(PRODUCTS_KEY) as Array<TProduct>;
    }
  ...
  async getAllRecipes() {
    if (this.cachingLayer.has(RECIPES_KEY)) {
      ...
      return this.cachingLayer.get(RECIPES_KEY) as Array<TRecipe>;
    }
